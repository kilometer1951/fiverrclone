...
var client = algoliasearch("36APM6O217", "ff4489512f16e6918e519cbc110313a6");
  ...
  .get((req, res, next) => {
    if (req.query.q) {
      index.search(req.query.q, function(err, content) {
        res.render('main/search_results', { content: content, search_results: req.query.q });
      });
    }
  })
  .post((req, res, next) => {
...
router.post('/api/add-promocode', (req, res, next) => {
